...
# Keep WB line only
df_WB = df[df['section']=='WB']

# Keep only one season <------------------------------------------------ ONE SEASON ONLY!!!
#df_WB = df_WB[(df_WB.index.month==6) | (df_WB.index.month==7) |  (df_WB.index.month==8)] 


# longitude range
idx = ((df_WB['Longitude']<-50))
df_WB = df_WB[idx]

# Keep only some variables
df_WB_O2 = df_WB[['depth','oxygen', 'Longitude', 'satO2']]
df_WB_nut = df_WB[['depth','PO4', 'NO3', 'SIO', 'Longitude']]
df_WB_nut.to_pickle('WB_nutrients')

# Drop NaNs
#df_WB_O2 = df_WB_O2.dropna(axis=0, how='any')


# depth range
idx = ((df_WB_O2['depth']>0) & (df_WB_O2['depth']<=125))
df_WB_O2_surf = df_WB_O2[idx]
idx = ((df_WB_nut['depth']>0) & (df_WB_nut['depth']<=125))
df_WB_nut_surf = df_WB_nut[idx]

idx = ((df_WB_O2['depth']>=200) & (df_WB_O2['depth']<=350))
df_WB_O2_btm = df_WB_O2[idx]
idx = ((df_WB_nut['depth']>200) & (df_WB_nut['depth']<=350))
df_WB_nut_btm = df_WB_nut[idx]


# resample
df_season_surf = df_WB_O2_surf.resample('4M').mean()
df_season_btm = df_WB_O2_btm.resample('4M').mean()


## ----  Build climatology ---- ##
df_O2clim = df_WB.groupby(['depth', 'sname'])['satO2%'].mean() # <---------- THIS IS SICK!!
df_NO3clim = df_WB.groupby(['depth', 'sname'])['NO3'].mean()
df_PO4clim = df_WB.groupby(['depth', 'sname'])['PO4'].mean()
df_SIOclim = df_WB.groupby(['depth', 'sname'])['SIO'].mean()
df_coords = df_WB.groupby(['sname'])['Latitude', 'Longitude'].mean()
...
# Load bathymetry
wb_bathy = '/home/cyrf0006/github/AZMP-NL/bathymetry/bottom_profiles/wbline.txt'
from numpy import loadtxt
bathy = loadtxt(wb_bathy, delimiter=",", unpack=False)
...
lon_array = np.array(df_WB_O2['Longitude'])
depth_array = np.array(df_WB_O2['depth'])
...
df_season_surf = df_WB_O2_surf.resample('4M').mean()
df_season_btm = df_WB_O2_btm.resample('4M').mean()
#df_season_surf = df_WB_O2_surf.resample('A').mean()
#df_season_btm = df_WB_O2_btm.resample('A').mean()
...
fig_name = 'WB_O2_timeseries.png'
fig.set_dpi(300)
fig.savefig(fig_name)


# NO3
df_season_surf = df_WB_nut_surf.resample('4M').mean()
df_season_btm = df_WB_nut_btm.resample('4M').mean()
#df_season_surf = df_WB_nut_surf.resample('A').mean()
#df_season_btm = df_WB_nut_btm.resample('A').mean()
...
plt.grid('on')
fig.set_size_inches(w=10,h=6)
fig_name = 'WB_NO3_timeseries.png'
fig.set_dpi(300)
fig.savefig(fig_name)
...
plt.grid('on')
fig.set_size_inches(w=10,h=6)
fig_name = 'WB_PO4_timeseries.png'
fig.set_dpi(300)
fig.savefig(fig_name)
...
plt.grid('on')
fig.set_size_inches(w=10,h=6)
fig_name = 'WB_SIO_timeseries.png'
fig.set_dpi(300)
fig.savefig(fig_name)
...
plt.grid('on')
fig.set_size_inches(w=10,h=6)
fig_name = 'WB_NO3onPO4_timeseries.png'
fig.set_dpi(300)
fig.savefig(fig_name)
...
plt.grid('on')
fig.set_size_inches(w=10,h=6)
fig_name = 'WB_NO3onSiO4_timeseries.png'
fig.set_dpi(300)
fig.savefig(fig_name)
