...
ds = xr.open_dataset('/home/cyrf0006/data/dev_database/viking_nc/2021_viking.nc')
...
XLIM = [datetime.date(2021, 4, 1), datetime.date(2021, 11, 1)]
...
c = plt.contourf(df_temp.index, np.array(df_temp.columns, dtype=float), df_temp.values.T, Vtemp, extend='both', cmap=plt.cm.RdBu_r)
cc = plt.contour(df_sig.index, np.array(df_sig.columns, dtype=float), df_sig.values.T, Vsig, colors='k')
...
## ---- plot Salinity ---- ##
fig, ax = plt.subplots(nrows=1, ncols=1)
c = plt.contourf(df_sal.index, np.array(df_sal.columns, dtype=float), df_sal.values.T, Vsal, extend='both', cmap=plt.cm.RdBu_r)
cc = plt.contour(df_sig.index, np.array(df_sig.columns, dtype=float), df_sig.values.T, Vsig, colors='k')
...
fig.set_dpi(200)
outfile = 'Viking2021_sal.png'
fig.savefig(outfile)
os.system('convert -trim ' + outfile + ' ' + outfile)

# Save Figure in French
outfile = 'Viking2021_sal_FR.png'
plt.ylabel('Profondeur (m)', fontsize=15, fontweight='bold')
fig.savefig(outfile, dpi=200)
os.system('convert -trim ' + outfile + ' ' + outfile)
