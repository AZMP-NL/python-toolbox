...
def variable_parameters(my_variable):
    ...
    if my_variable == 'pH_Total_(total_scale)':
        return [9, 7.4, 8.2, 7.8, cmo.thermal_r, [7.4, 7.5, 7.6, 7.7, 7.8, 7.9, 8.0, 8.1, 8.2], r'$\rm pH_{T,is}$', 'both', -.5, .5, 11]
...
#        return [25, -2, 22, 12, plt.cm.plasma, [-2, 2, 6, 10, 14, 18, 22], r'$\theta\ (^\circ$C)', 'both']
        ...
        return [14, -2, 24, 12, cmo.thermal, [-2, 2, 6, 10, 14, 18, 22, 26], r'$\theta\ (^\circ$C)', 'both', -3, 3, 13]

    elif my_variable == 'Omega_Aragonite_(unitless)':
        return [21, 0, 4, 1, plt.cm.seismic_r, [0, 1, 2, 3, 4], r'$\Omega_{\rm{a}}$', 'both', -.25, .25, 11]

    elif my_variable == 'Salinity_(psu)':
        return [21, 21, 37, 29, cmo.haline, [21, 25, 29, 33, 37], r'S', 'both']
...
#        return [21, 27, 37, 32, cmo.haline, [27, 29, 31, 33, 35, 37], r'Salinity', 'both']
        ...
        return [11, 0, 4, 0.5, plt.cm.seismic_r, [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4], r'$\rm O_{2,sat}$ (%)', 'both', -10, 10, 11]
...
#        return [11, 50, 100, 75, plt.cm.plasma, [50,60,70,80,90,100], r'$\rm O_{2,sat}$ (%)', 'both']
        ...
        return [11, 300, 800, 500, cmo.thermal_r, [250,500,750,1000,1250], r'$\rm pCO_{2} (\rm \mu $atm)', 'both', -10, 10, 11]
    
    elif my_variable == 'chla':
        return [11, 0, 2.5, 1.25, cmo.algae, [0,0.5,1,1.5,2,2.5], r'Chlorophyll a (mg/m$^{3}$))', 'both']
    
    elif my_variable == 'Omega_Calcite_(unitless)':
        return [21, 0, 5, 1, plt.cm.seismic_r, [0, 1, 2, 3, 4, 5], r'$\Omega_{\rm{c}}$', 'both']
