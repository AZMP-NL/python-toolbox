...
#    
    ...
    ax.add_feature(cpf.NaturalEarthFeature('physical', 'coastline', '10m', edgecolor='k', alpha=0.7, linewidth=0.6, facecolor='black'))#cpf.COLORS['land']))
    ...
    c = plt.contourf(lon, lat, -Z, v, transform=ccrs.PlateCarree(), cmap=lightdeep, extend='max', zorder=1)
    cc = plt.contour(lon, lat, -Z, [100, 500, 1000, 2000, 3000, 4000, 5000], colors='lightgrey', linewidths=.5, transform=ccrs.PlateCarree(), zorder=10)
    ...
    #######################plot data onto map######################################
    s = ax.scatter(lon_data,lat_data, c=data, s=40, lw=0.3, edgecolor='black', vmin=vmin, vmax=vmax, cmap=cmap, transform=ccrs.Geodetic(), zorder=10)
    ...
    fig.savefig('C:\Users\gibbo\Documents\carbonates\AZMP_OA\AZMP_OA data\AZMP_OA plots\AZMP_OA maps\AZMP_OA maps\AZMP_OA_'+my_season+'_'+my_variable+'_'+my_depth+'.png', 
